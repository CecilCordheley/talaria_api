...
abstract class AsyncTicket
{
    ...
    public static function getKPIByState()
    {
        try {
            $sqlF = self::getSQLFactory();
            $user = self::checkToken(["admin"]);
            return $sqlF->execQuery("SELECT * FROM `ticket_durations`");
    ...
    public static function getKPIByService()
    {
        try {
            $sqlF = self::getSQLFactory();
            self::checkToken(["admin"]);

            $data = $sqlF->execQuery("SELECT * FROM `ticket_by_service`");

            $result = array_reduce($data, function ($carry, $row) {

                $service = $row['nomService'];
                $state = $row['libEtatTicket'];
                $count = (int) $row['total'];

                if (!isset($carry[$service])) {
                    $carry[$service] = [
                        "service" => $service,
                        "total" => 0,
                        "states" => []
                ...
                // Ajout du nombre par état
                $carry[$service]["states"][$state] = $count;

                // Somme globale
                $carry[$service]["total"] += $count;

                return $carry;

            }, []);

            // Réindexer (important pour JSON propre)
    ...
    public static function assignAgent($ticket, $agent, $comment = "")
    {
        try {
            $sqlF = self::getSQLFactory();
            $user = self::checkToken(["manager", "admin"]);
            // --- Récupération du ticket ---
            $ticket = TicketEntity::getTicketBy($sqlF, "uuidTicket", $ticket);
            if ($ticket == false) {
                echo json_encode(["result" => "error", "message" => "no ticket found"]);
                exit();
            }
            if ($ticket->assignAgent($sqlF, $agent)) {
    ...
    public static function getTickets($filter = null)
    {
        try {
            $sqlF = self::getSQLFactory();
            $user = self::checkToken(["manager", "agent", "dev", "admin"]);
            ...
            $return2_arr = array_reduce($tickets, function ($carry, $ticket) use (&$i, $sqlF) {
                ...
                // États
                $i = 0;
                $t["states"] = array_reduce($ticket->getEtats($sqlF), function ($c, $s) use (&$i, $sqlF) {
                    $c[$i] = $s->getArray();
    ...
    public static function updateTicket($idTiket)
    {
        try {
            ...
            $ticket->prioriteTicket = $jsonData["priority"] ?? $ticket->prioriteTicket;
    ...
    public static function createTicket()
    {
        try {
            ...
            $ticket->entreprise_source = $user->getEntreprise($sqlF)[0]["idEntreprise"];
            ...
            $ticket->typeTicket = $type->idTypeTicket;
            $ticket->service = $service->idService;
            $ticket->entreprise_cible = $service->Entreprise;
            if (
                TicketEntity::add($sqlF, $ticket, function (TicketEntity $ticket) use ($sqlF) {
